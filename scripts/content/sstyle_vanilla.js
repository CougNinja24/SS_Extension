(function () {
  ...
  document
    .querySelector("button#refreshSearch")
    .addEventListener("click", (evt) => {
      let color = getColor().topRow;
      let renameBox = document.querySelector("#checkname");
      if (renameBox.checked === false) {
        setTabName();
      }

      // Set the color of the main control bar background
      document.querySelector(
        "#gameManager > div:nth-child(3)"
  ...
  document
    .querySelector("#gameManager > div:nth-child(3) > h4")
    .addEventListener("click", function () {
      console.log("Event Handler for Controls H4 activated");
      document.querySelector(
        "div#searchResults tr.gameRow td"
      ).style.backgroundColor = getColor().bottomRows;
    });
  ...
  function getGender() {
    ...
    let genderStr = document.querySelector(
      "#s2id_squadFilter > ul"
    ).textContent;
  ...
  function createRenameCheckBox() {
    ...
    document.querySelector("span#clearQuickFind").append(box);
  }
